...
double Tiempo()
{
	...
	ftime(&tiempob) ;
	return (double)(tiempob.time+0.001*tiempob.millitm) ;
}
/******************************************************************************/
int random(int k)
{
...
int MouseHit(int *x,int *y)
  ...
  MSG msg;
  PeekMessage(&msg,NULL,0,0,PM_REMOVE);
  if (msg.message==WM_RBUTTONDOWN || msg.message==WM_MOUSEMOVE || msg.message==WM_LBUTTONDOWN)
  //if (msg.message==WM_MOUSEMOVE || msg.message==WM_LBUTTONDOWN) //mousehitcontinuo
  {
     *x=LOWORD(msg.lParam);
     *y=HIWORD(msg.lParam);
  }
  if(msg.message==WM_LBUTTONDOWN){
   ...
   PostMessage(msg.hwnd,WM_LBUTTONUP,0,0);//mousehit continuo
   return 1;
   
	}
  if(msg.message==WM_RBUTTONDOWN){
  	PostMessage(msg.hwnd,WM_RBUTTONUP,0,0);//mousehit continuo
   return 2;
	}
  return 0;
  }
